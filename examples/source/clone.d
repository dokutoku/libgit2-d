...
extern (C)
nothrow @nogc
private int fetch_progress(const (libgit2.indexer.git_indexer_progress)* stats, void* payload)
...
extern (C)
nothrow @nogc
public int lg2_clone(libgit2.types.git_repository* repo, int argc, char** argv)
	...
	do
	{
		...
		libgit2.clone.git_clone_options clone_opts = libgit2.clone.GIT_CLONE_OPTIONS_INIT();
		clone_opts.checkout_opts = checkout_opts;
		clone_opts.fetch_opts.callbacks.sideband_progress = &.sideband_progress;
		clone_opts.fetch_opts.callbacks.transfer_progress = &.fetch_progress;
		clone_opts.fetch_opts.callbacks.credentials = &libgit2.example.common.cred_acquire_cb;
		clone_opts.fetch_opts.callbacks.payload = &pd;

		/* Do the clone */
		libgit2.types.git_repository* cloned_repo = null;
		int error = libgit2.clone.git_clone(&cloned_repo, url, path, &clone_opts);
		...
		if (error != 0) {
			...
			libgit2.repository.git_repository_free(cloned_repo);
		}
