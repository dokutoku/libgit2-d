...
nothrow @nogc
private void parse_options(const (char)** repo_path, .checkout_options* opts, libgit2.example.args.args_info* args)
	...
	do
	{
		...
		for (args.pos = 1; args.pos < args.argc; ++args.pos) {
			...
			if (libgit2.example.args.match_arg_separator(args)) {
				break;
			} else if (!core.stdc.string.strcmp(curr, "--force")) {
				opts.force = 1;
			} else if (libgit2.example.args.match_bool_arg(&bool_arg, args, "--progress")) {
				opts.progress = bool_arg;
			} else if (libgit2.example.args.match_bool_arg(&bool_arg, args, "--perf")) {
				opts.perf = bool_arg;
...
extern (C)
nothrow @nogc
private void print_perf_data(const (libgit2.checkout.git_checkout_perfdata)* perfdata, void* payload)
 ...
 */
nothrow @nogc
private int perform_checkout_ref(libgit2.types.git_repository* repo, const (char)* target_ref, libgit2.types.git_annotated_commit* target, .checkout_options* opts)
	...
	do
	{
		...
		checkout_opts.checkout_strategy = libgit2.checkout.git_checkout_strategy_t.GIT_CHECKOUT_SAFE;

		if (opts.force) {
			checkout_opts.checkout_strategy = libgit2.checkout.git_checkout_strategy_t.GIT_CHECKOUT_FORCE;
		}
		...
		libgit2.types.git_commit* target_commit = null;

		scope (exit) {
			libgit2.commit.git_commit_free(target_commit);
			libgit2.commit.git_commit_free(cast(libgit2.types.git_commit*)(ref_));
		}

		int err = libgit2.commit.git_commit_lookup(&target_commit, repo, libgit2.annotated_commit.git_annotated_commit_id(target));
		 ...
		 * peeled to a tree.
		 */
		err = libgit2.checkout.git_checkout_tree(repo, cast(const (libgit2.types.git_object)*)(target_commit), &checkout_opts);
		 ...
		 */
		libgit2.types.git_reference* branch = null;

		scope (exit) {
			libgit2.refs.git_reference_free(branch);
		}

		if (libgit2.annotated_commit.git_annotated_commit_ref(target)) {
			err = libgit2.refs.git_reference_lookup(&ref_, repo, libgit2.annotated_commit.git_annotated_commit_ref(target));
			...
			if (libgit2.refs.git_reference_is_remote(ref_)) {
				err = libgit2.branch.git_branch_create_from_annotated(&branch, repo, target_ref, target, 0);

				if (err < 0) {
					goto error;
				}

				target_head = libgit2.refs.git_reference_name(branch);
			} else {
				target_head = libgit2.annotated_commit.git_annotated_commit_ref(target);
			}

			err = libgit2.repository.git_repository_set_head(repo, target_head);
		} else {
			err = libgit2.repository.git_repository_set_head_detached_from_annotated(repo, target);
		}
 ...
 */
nothrow @nogc
private int guess_refish(libgit2.types.git_annotated_commit** out_, libgit2.types.git_repository* repo, const (char)* ref_)

	do
	{
		libgit2.strarray.git_strarray remotes =
		{
			null,
			0,
		};
		libgit2.types.git_reference* remote_ref = null;

		scope (exit) {
			libgit2.refs.git_reference_free(remote_ref);
			libgit2.strarray.git_strarray_dispose(&remotes);
		}

		int error = libgit2.remote.git_remote_list(&remotes, repo);
		...
		for (size_t i = 0; i < remotes.count; i++) {
			char* refname = null;
			size_t reflen = libgit2.example.common.snprintf(refname, 0, "refs/remotes/%s/%s", remotes.strings[i], ref_);
			...
			libgit2.example.common.snprintf(refname, reflen + 1, "refs/remotes/%s/%s", remotes.strings[i], ref_);
			error = libgit2.refs.git_reference_lookup(&remote_ref, repo, refname);
	...
	next:
			...
			if ((error < 0) && (error != libgit2.errors.git_error_code.GIT_ENOTFOUND)) {
				break;
			}
		}

		if (!remote_ref) {
			error = libgit2.errors.git_error_code.GIT_ENOTFOUND;
			return error;
		}

		if ((error = libgit2.annotated_commit.git_annotated_commit_from_ref(out_, repo, remote_ref)) < 0) {
			return error;
		}
...
extern (C)
nothrow @nogc
public int lg2_checkout(libgit2.types.git_repository* repo, int argc, char** argv)
	...
	do
	{
		libgit2.types.git_annotated_commit* checkout_target = null;
		int err = 0;
		const (char)* path = ".";

		scope (exit) {
			libgit2.annotated_commit.git_annotated_commit_free(checkout_target);
		}
		...
		if (libgit2.example.args.match_arg_separator(&args)) {
			 ...
			 */
			if (((err = libgit2.example.common.resolve_refish(&checkout_target, repo, args.argv[args.pos])) < 0) && ((err = .guess_refish(&checkout_target, repo, args.argv[args.pos])) < 0)) {
			...
			err = .perform_checkout_ref(repo, cast(const (char)*)(checkout_target), cast(libgit2.types.git_annotated_commit*)(args.argv[args.pos]), &opts);
		}
