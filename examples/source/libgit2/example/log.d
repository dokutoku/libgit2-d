...
private static import libgit2.pathspec;
...
public struct log_state
{
	...
	const (char)* repodir;
	libgit2.types.git_revwalk* walker;
	int hide;
	int sorting;
	int revisions;
}
...
public struct log_options
{
	...
	int min_parents;
	int max_parents;
	libgit2.types.git_time_t before;
	libgit2.types.git_time_t after;
	const (char)* author;
...
public int lg2_log(libgit2.types.git_repository* repo, int argc, char** argv)
	...
	do
	{
		...
		libgit2.diff.git_diff_options diffopts = libgit2.diff.GIT_DIFF_OPTIONS_INIT();
		diffopts.pathspec.strings = &argv[last_arg];
		diffopts.pathspec.count = argc - last_arg;

		libgit2.pathspec.git_pathspec* ps = null;

		if (diffopts.pathspec.count > 0) {
			libgit2.example.common.check_lg2(libgit2.pathspec.git_pathspec_new(&ps, &diffopts.pathspec), "Building pathspec", null);
		}
		...
		libgit2.types.git_commit* commit = null;
		libgit2.types.git_tree* tree;
		libgit2.types.git_commit* parent;

		for (; !libgit2.revwalk.git_revwalk_next(&oid, s.walker); libgit2.commit.git_commit_free(commit)) {
			libgit2.example.common.check_lg2(libgit2.commit.git_commit_lookup(&commit, s.repo, &oid), "Failed to look up commit", null);

			int parents = cast(int)(libgit2.commit.git_commit_parentcount(commit));
			...
			if (diffopts.pathspec.count > 0) {
				...
				if (parents == 0) {
					libgit2.example.common.check_lg2(libgit2.commit.git_commit_tree(&tree, commit), "Get tree", null);

					if (libgit2.pathspec.git_pathspec_match_tree(null, tree, libgit2.pathspec.git_pathspec_flag_t.GIT_PATHSPEC_NO_MATCH_ERROR, ps) != 0) {
						unmatched = 1;
					}
			...
			if (!.signature_matches(libgit2.commit.git_commit_author(commit), opt.author)) {
				continue;
			}

			if (!.signature_matches(libgit2.commit.git_commit_committer(commit), opt.committer)) {
				continue;
			}
			...
			if (opt.show_diff) {
				libgit2.types.git_tree* a = null;
				libgit2.types.git_tree* b = null;
				libgit2.diff.git_diff* diff = null;

				if (parents > 1) {
					continue;
				}

				libgit2.example.common.check_lg2(libgit2.commit.git_commit_tree(&b, commit), "Get tree", null);

				if (parents == 1) {
					libgit2.example.common.check_lg2(libgit2.commit.git_commit_parent(&parent, commit, 0), "Get parent", null);
					libgit2.example.common.check_lg2(libgit2.commit.git_commit_tree(&a, parent), "Tree for parent", null);
					libgit2.commit.git_commit_free(parent);
				}

				libgit2.example.common.check_lg2(libgit2.diff.git_diff_tree_to_tree(&diff, libgit2.commit.git_commit_owner(commit), a, b, &diffopts), "Diff commit with parent", null);
				libgit2.example.common.check_lg2(libgit2.diff.git_diff_print(diff, libgit2.diff.git_diff_format_t.GIT_DIFF_FORMAT_PATCH, &libgit2.example.common.diff_output, null), "Displaying diff", null);

				libgit2.diff.git_diff_free(diff);
				libgit2.tree.git_tree_free(a);
				libgit2.tree.git_tree_free(b);
			}
		}

		libgit2.pathspec.git_pathspec_free(ps);
		libgit2.revwalk.git_revwalk_free(s.walker);

		return 0;
	}

/**
 * Determine if the given libgit2.types.git_signature does not contain the filter text.
 */
nothrow @nogc
...
private int log_message_matches(const (libgit2.types.git_commit)* commit, const (char)* filter)
	...
	do
	{
		...
		message = libgit2.commit.git_commit_message(commit);
...
private void push_rev(.log_state* s, libgit2.types.git_object* obj, int hide)
	...
	do
	{
		...
		if (s.walker == null) {
			libgit2.example.common.check_lg2(libgit2.revwalk.git_revwalk_new(&s.walker, s.repo), "Could not create revision walker", null);
			libgit2.revwalk.git_revwalk_sorting(s.walker, s.sorting);
		}

		if (obj == null) {
			libgit2.example.common.check_lg2(libgit2.revwalk.git_revwalk_push_head(s.walker), "Could not find repository HEAD", null);
		} else if (hide) {
			libgit2.example.common.check_lg2(libgit2.revwalk.git_revwalk_hide(s.walker, libgit2.object.git_object_id(obj)), "Reference does not refer to a commit", null);
		} else {
			libgit2.example.common.check_lg2(libgit2.revwalk.git_revwalk_push(s.walker, libgit2.object.git_object_id(obj)), "Reference does not refer to a commit", null);
		}
...
private int add_revision(.log_state* s, const (char)* revstr)
	...
	do
	{
		libgit2.revparse.git_revspec revs;
		int hide = 0;
		...
		} else if (libgit2.revparse.git_revparse(&revs, s.repo, revstr) < 0) {
			return -1;
		}
...
private void set_sorting(.log_state* s, uint sort_mode)
	...
	do
	{
		...
		if (s.repo == null) {
			...
			libgit2.example.common.check_lg2(libgit2.repository.git_repository_open_ext(&s.repo, s.repodir, 0, null), "Could not open repository", s.repodir);
		}
		...
		if (sort_mode == libgit2.revwalk.git_sort_t.GIT_SORT_REVERSE) {
			s.sorting = s.sorting ^ libgit2.revwalk.git_sort_t.GIT_SORT_REVERSE;
		} else {
			s.sorting = sort_mode | (s.sorting & libgit2.revwalk.git_sort_t.GIT_SORT_REVERSE);
		}
...
/**
 * Helper to format a libgit2.types.git_time value like Git.
 */
nothrow @nogc
...
private void print_commit(libgit2.types.git_commit* commit, .log_options* opts)
	...
	do
	{
		...
		int count = cast(int)(libgit2.commit.git_commit_parentcount(commit));

		if (count > 1) {
			core.stdc.stdio.printf("Merge:");

			for (int i = 0; i < count; ++i) {
				libgit2.oid.git_oid_tostr(&(buf[0]), 8, libgit2.commit.git_commit_parent_id(commit, i));
		...
		const (libgit2.types.git_signature)* sig = libgit2.commit.git_commit_author(commit);
		...
		for (scan = libgit2.commit.git_commit_message(commit); (scan) && (*scan); ) {
 ...
 */
nothrow @nogc
private int match_with_parent(libgit2.types.git_commit* commit, int i, libgit2.diff.git_diff_options* opts)
	...
	{
		libgit2.types.git_commit* parent;
		libgit2.types.git_tree* a;
		libgit2.types.git_tree* b;
		libgit2.diff.git_diff* diff;

		libgit2.example.common.check_lg2(libgit2.commit.git_commit_parent(&parent, commit, cast(size_t)(i)), "Get parent", null);
		libgit2.example.common.check_lg2(libgit2.commit.git_commit_tree(&a, parent), "Tree for parent", null);
		libgit2.example.common.check_lg2(libgit2.commit.git_commit_tree(&b, commit), "Tree for commit", null);
		libgit2.example.common.check_lg2(libgit2.diff.git_diff_tree_to_tree(&diff, libgit2.commit.git_commit_owner(commit), a, b, opts), "Checking diff between parent and commit", null);

		int ndeltas = cast(int)(libgit2.diff.git_diff_num_deltas(diff));
...
private int parse_options(.log_state* s, .log_options* opt, int argc, char** argv)
	...
	do
	{
		...
		s.sorting = libgit2.revwalk.git_sort_t.GIT_SORT_TIME;
		...
		for (args.pos = 1; args.pos < argc; ++args.pos) {
			...
			if (a[0] != '-') {
				...
				.set_sorting(s, libgit2.revwalk.git_sort_t.GIT_SORT_TIME);
			} else if (!core.stdc.string.strcmp(a, "--topo-order")) {
				.set_sorting(s, libgit2.revwalk.git_sort_t.GIT_SORT_TOPOLOGICAL);
			} else if (!core.stdc.string.strcmp(a, "--reverse")) {
				.set_sorting(s, libgit2.revwalk.git_sort_t.GIT_SORT_REVERSE);
			} else if (libgit2.example.args.match_str_arg(&opt.author, &args, "--author")) {
				/** Found valid --author */
			} else if (libgit2.example.args.match_str_arg(&opt.committer, &args, "--committer")) {
				/** Found valid --committer */
			} else if (libgit2.example.args.match_str_arg(&opt.grep, &args, "--grep")) {
				/** Found valid --grep */
			} else if (libgit2.example.args.match_str_arg(&s.repodir, &args, "--git-dir")) {
				/** Found git-dir. */
			} else if (libgit2.example.args.match_int_arg(&opt.skip, &args, "--skip", 0)) {
				/** Found valid --skip. */
			} else if (libgit2.example.args.match_int_arg(&opt.limit, &args, "--max-count", 0)) {
				/** Found valid --max-count. */
			} else if ((a[1] >= '0') && (a[1] <= '9')) {
				libgit2.example.args.is_integer(&opt.limit, a + 1, 0);
			} else if (libgit2.example.args.match_int_arg(&opt.limit, &args, "-n", 0)) {
				/** Found valid -n. */
				...
				opt.max_parents = -1;
			} else if (libgit2.example.args.match_int_arg(&opt.max_parents, &args, "--max-parents=", 1)) {
				/** Found valid --max-parents. */
			} else if (libgit2.example.args.match_int_arg(&opt.min_parents, &args, "--min-parents=", 0)) {
