...
extern (C)
nothrow @nogc
private int update_cb(const (char)* refname, const (libgit2.oid.git_oid)* a, const (libgit2.oid.git_oid)* b, void* data)
...
extern (C)
nothrow @nogc
private int transfer_progress_cb(const (libgit2.indexer.git_indexer_progress)* stats, void* payload)
	...
	do
	{
		...
		if (stats.received_objects == stats.total_objects) {
			...
			core.stdc.stdio.printf("Received %u/%u objects (%u) in %" ~ libgit2.example.common.PRIuZ ~ " bytes\r", stats.received_objects, stats.total_objects, stats.indexed_objects, stats.received_bytes);
		}
...
extern (C)
nothrow @nogc
public int lg2_fetch(libgit2.types.git_repository* repo, int argc, char** argv)
	...
	do
	{
		...
		if (libgit2.remote.git_remote_lookup(&remote, repo, argv[1]) < 0) {
			if (libgit2.remote.git_remote_create_anonymous(&remote, repo, argv[1]) < 0) {
				return -1;
			}
		}

		/* Set up the callbacks (only update_tips for now) */
		libgit2.remote.git_fetch_options fetch_opts = libgit2.remote.GIT_FETCH_OPTIONS_INIT();
		fetch_opts.callbacks.update_tips = &.update_cb;
		fetch_opts.callbacks.sideband_progress = &.progress_cb;
		fetch_opts.callbacks.transfer_progress = &.transfer_progress_cb;
		fetch_opts.callbacks.credentials = &libgit2.example.common.cred_acquire_cb;
		 ...
		 * "fetch".
		 */
		if (libgit2.remote.git_remote_fetch(remote, null, &fetch_opts, "fetch") < 0) {
			return -1;
		}
		 ...
		 * network.
		 */
		const (libgit2.indexer.git_indexer_progress)* stats = libgit2.remote.git_remote_stats(remote);

		if (stats.local_objects > 0) {
			core.stdc.stdio.printf("\rReceived %u/%u objects in %" ~ libgit2.example.common.PRIuZ ~ " bytes (used %u local objects)\n", stats.indexed_objects, stats.total_objects, stats.received_bytes, stats.local_objects);
		} else {
			core.stdc.stdio.printf("\rReceived %u/%u objects in %" ~ libgit2.example.common.PRIuZ ~ "bytes\n", stats.indexed_objects, stats.total_objects, stats.received_bytes);
		}
