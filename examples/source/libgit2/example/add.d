...
extern (C)
nothrow @nogc
public int lg2_add(libgit2.types.git_repository* repo, int argc, char** argv)
	...
	do
	{
		...
		libgit2.strarray.git_strarray array = libgit2.strarray.git_strarray.init;
		libgit2.example.args.strarray_from_args(&array, &args);
		...
		libgit2.index.git_index_matched_path_cb matched_cb = null;
		...
		if (options.add_update) {
			libgit2.index.git_index_update_all(index, &array, matched_cb, &options);
		} else {
			libgit2.index.git_index_add_all(index, &array, 0, matched_cb, &options);
		}

		/* Cleanup memory */
		libgit2.index.git_index_write(index);
...
public int print_matched_cb(const (char)* path, const (char)* matched_pathspec, void* payload)
	...
	do
	{
		...
		/* Get the file status */
		if (libgit2.status.git_status_file(&status, opts.repo, path) < 0) {
			return -1;
		}

		int ret;

		if ((status & libgit2.status.git_status_t.GIT_STATUS_WT_MODIFIED) || (status & libgit2.status.git_status_t.GIT_STATUS_WT_NEW)) {
...
nothrow @nogc
private void parse_opts(const (char)** repo_path, .index_options* opts, libgit2.example.args.args_info* args)
	...
	do
	{
		...
		for (args.pos = 1; args.pos < args.argc; ++args.pos) {
			...
			} else if ((libgit2.example.args.match_bool_arg(&opts.verbose, args, "--verbose")) || (libgit2.example.args.match_bool_arg(&opts.dry_run, args, "--dry-run")) || (libgit2.example.args.match_str_arg(repo_path, args, "--git-dir")) || ((opts.mode == .index_mode.INDEX_ADD) && (libgit2.example.args.match_bool_arg(&opts.add_update, args, "--update")))) {
				continue;
			} else if (libgit2.example.args.match_bool_arg(null, args, "--help")) {
				.print_usage();
